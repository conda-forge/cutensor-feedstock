...
if [[ $target_platform == "linux-64" ]]; then
    ...
    error_log=$( nvcc --verbose ${NVCC_FLAGS} --std=c++11 -I${PREFIX}/include -L${PREFIX}/lib -lcutensor -lcudart contraction.cu -o contraction 2>&1 )
    echo "${error_log}"
    error_log=$( nvcc --verbose ${NVCC_FLAGS} --std=c++11 -I${PREFIX}/include -L${PREFIX}/lib -lcutensor -lcudart reduction.cu -o reduction 2>&1 )
    echo "${error_log}"
    cd ../cuTENSORMg/
    error_log=$( nvcc --verbose ${NVCC_FLAGS} --std=c++11 -I${PREFIX}/include -L${PREFIX}/lib -lcutensorMg -lcutensor -lcudart contraction_multi_gpu.cu -o contraction_multi_gpu 2>&1 )
    echo "${error_log}"
fi
